import {
  Box, Button, IconButton, TextField, MenuItem, Switch,
...
const FIELD_TYPES: ConfigField['type'][] = [
  'text', 'password', 'select', 'number', 'boolean', 'textarea',
];
...
const ConfigSchemaBuilder = ({ fields, onChange }: Props) => {
  ...
  const updateOption = (fIdx: number, oIdx: number, key: 'label' | 'value', val: string) => {
    const f = fields[fIdx];
      ...
      {fields.map((field, idx) => (
        <Paper key={idx} variant="outlined" sx={{ p: 1.5, mb: 1.5 }}>
          <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', alignItems: 'center' }}>
            <TextField size="small" label="Key" value={field.key} sx={{ flex: 1, minWidth: 100 }}
              onChange={(e) => updateField(idx, { key: e.target.value })} />
            <TextField size="small" label="Label" value={field.label} sx={{ flex: 1, minWidth: 100 }}
              onChange={(e) => updateField(idx, { label: e.target.value })} />
            <TextField size="small" select label="Type" value={field.type} sx={{ minWidth: 110 }}
          ...
          <Box sx={{ display: 'flex', gap: 1, mt: 1, flexWrap: 'wrap' }}>
            <TextField size="small" label="Placeholder" value={field.placeholder} sx={{ flex: 1 }}
              onChange={(e) => updateField(idx, { placeholder: e.target.value })} />
            <TextField size="small" label="Default Value" value={field.defaultValue} sx={{ flex: 1 }}
              onChange={(e) => updateField(idx, { defaultValue: e.target.value })} />
          </Box>
          {field.type === 'select' && (
            <Box sx={{ mt: 1 }}>
              <Divider sx={{ mb: 1 }} />
              <Typography variant="caption" sx={{ mb: 0.5, display: 'block' }}>Options</Typography>
              {field.options.map((opt, oIdx) => (
                <Box key={oIdx} sx={{ display: 'flex', gap: 1, mb: 0.5, alignItems: 'center' }}>
                  <TextField size="small" label="Label" value={opt.label}
                    onChange={(e) => updateOption(idx, oIdx, 'label', e.target.value)} sx={{ flex: 1 }} />
                  <TextField size="small" label="Value" value={opt.value}
                    onChange={(e) => updateOption(idx, oIdx, 'value', e.target.value)} sx={{ flex: 1 }} />
                  <IconButton size="small" onClick={() => removeOption(idx, oIdx)}>
                    <DeleteIcon fontSize="small" />
                  </IconButton>
                </Box>
              ))}
              <Button size="small" onClick={() => addOption(idx)}>Add Option</Button>
            </Box>
          )}
        </Paper>
      ))}
